...
var (
	modwintun                         = newLazyDLL("wintun.dll", nil)
...
func CreateAdapter(name string, tunnelType string,
	requestedGUID *windows.GUID) (wintun *Adapter, err error) {
	var name16 *uint16
...
func OpenAdapter(name string) (wintun *Adapter, err error) {
	...
	r0, _, e1 := syscall.SyscallN(procWintunOpenAdapter.Addr(),
		uintptr(unsafe.Pointer(name16)))
	if r0 == 0 {
		err = e1
		return
...
func (wintun *Adapter) LUID() (luid uint64) {
	syscall.SyscallN(procWintunGetAdapterLUID.Addr(),
		uintptr(wintun.handle), uintptr(unsafe.Pointer(&luid)))
	return
}
